...
def calculate_seasonal_means_yearly(netcdf_dir, variable, shapefile_path, season, output_csv):
    ...
    for netcdf_file in netcdf_files:
        ...
        # Convert cumulative variables to daily totals
        if variable in CONVERT_TO_MM:
            data = data.differentiate(coord="valid_time")  # Calculate daily differences
            data *= 1000  # Convert from meters to millimeters
            units = "mm"  # Update the units
        ...
        temp_raster = f"temp_raster_{variable}_{season}_{year}.tif"
        seasonal_stat.rio.to_raster(temp_raster)

        # Perform zonal statistics
        stats = zonal_stats(
            gdf,
            temp_raster,
            stats=["mean", "min", "max"],  # Include min and max
            all_touched=True,
            geojson_out=True,
            nodata=-999,  # Explicitly ignore nodata values
        )
